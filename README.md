...
*   **`bootc`**: A tool for building and managing bootable container images. It uses two container images: one as a base for the build, and another to create the final artifacts (ISO, OVA, etc.). For more information, see the [official Red Hat documentation](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/9/html/composing_installing_and_managing_rhel_for_the_edge/getting-started-with-bootc_composing-rhel-for-the-edge).
...
*   **`kustomize`**: [Integrated as part of MicroShift](https://docs.redhat.com/en/documentation/red_hat_build_of_microshift/4.19/html/running_applications/applications-with-microshift), it is used in the `wordpress` example for managing Kubernetes manifests.
